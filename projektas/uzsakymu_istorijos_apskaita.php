...
if(DB_SERVER === null)
	...
	$date=$_POST["vardas"];
	$date2=$_POST["epastas"];
	$klientas=$_POST["klientas"];
	
	function getKlientas() {
		$klientass=$_POST["klientas"];
		return $klientass;
	}
	...
	$date=$_POST["vardas"];
	$date2=$_POST["epastas"];
	$klientas2=$_POST["klientas"];
	
	$useriai = mysqli_query($dbc, "SELECT `klientas`.`id`,
										  `klientas`.`vardas`,
										  `klientas`.`pavarde`
									FROM `klientas`
									WHERE `klientas`.`id`='$klientas2'");
	$useris = mysqli_fetch_assoc($useriai);
           ...
           <center><img src="pictures/top.png"/></center>
            ...
            if ($session->logged_in) {
                ...
                <div style="text-align:left;color:green;font-size: 30px">
                    <br><br>
                    <p style='padding-left : 30px'>Klientų užsakymai <?php ?></p>
						<div style="text-align:left;color:green;font-size: 16px">
                            <p style='padding-left : 30px'><u>Išsirinkite klientą ir užsakymų istorijos laikotarpį</u></p>
                                <div class="container">
                                    <form method='post'>
										<p style="padding-left:14px;"><b>Klientas:</b>
                                        <select id="username" name="klientas">
										<option value="-1">Pasirinkite..</option>
											<?php
												// išrenkame visus klientus
												$klientas = getKlientai();
												foreach($klientas as $key => $val) {
													$selected = "";
													if(isset($fields['klientas']) && $fields['klientas'] == $val['id']) {
														$selected = " selected='selected'";
													}
													echo "<option{$selected} value='{$val['id']}'>{$val['slapyvardis']}</option>";
													echo $form->value($val['id']);
												}
											?>
										</select>
						...
						if (mysqli_num_rows($irasai) == 0) { ?>
						<div style="text-align:left;color:green;font-size: 20px">
							<p style='padding-left : 30px'><u>Klientas neturi jokių sudarytų užsakymų.</u></p>
						</div>
						<?php } else { ?>
						<div class="datagrid">
						<table class="lentele">
						...
						<tr>
							...
							<td> <a href="uzsakymo_busenos_perziura.php?id=<?php $kl = getKlientas(); echo $kl; ?>&id2=<?php echo $irasas['uzsakymoNr']; ?>" <?php echo "<div style='text-align:left'>"."daugiau..."."</div>"; ?> </a> </td>
							<td> <a href='uzsakymo_salinimas.php?e=1&deluzsakymas=<?php echo $irasas['uzsakymoNr']; ?>' onclick="return confirm_delete();">Trinti</a></td>
						</tr>
						<?php endwhile; ?>
						</tbody>
						</table>
			...
			<?php } } ?>
					...
					<p style='padding-left : 30px'>Klientų užsakymų apskaita</p>
					
						<div style="text-align:left;color:green;font-size: 16px">
                            <p style='padding-left : 30px'><u>Išsirinkite klientą ir užsakymų apskaitos laikotarpį</u></p>
                                <div class="container">
                                    <form method='post'>
										<p style="padding-left:14px;"><b>Klientas:</b>
                                        <select id="username" name="klientas">
